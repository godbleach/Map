...
validatePost = function (post) {
	...
	if(!post.stdId)
		errors.stuId = "Please fill StudentID ";
	if(!post.userName)
...
Meteor.methods({
	...
	postInsert: function(postAttributes) {
		...
		if (errors.stdId || errors.userName || errors.firstName || errors.lastName || errors.bloodType || errors.faculty || errors.major || errors.emerCell)
	...
	getData: function(id){
		return Posts.findOne({userId:id});
	},

});

if (Meteor.isServer) {
	Meteor.publish('posts.all', () => Posts.find().cursor);
  } else {
